...
def _fetch_url_as_base64(url: str) -> str:
    ...
    The URL is user-supplied, so a 404 or a dead host is ordinary rather
    than exceptional and must not surface as a traceback.
    """
    ...
    try:
        ...
        raise RuntimeError(f"could not fetch {url}: HTTP {e.code}") from e
