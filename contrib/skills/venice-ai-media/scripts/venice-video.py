...
DEFAULT_MODEL = load_config().get("video", {}).get("model", "")
...
def get_video_quote(
    ...
    return api_json("/video/quote", "POST", payload, api_key, timeout=30)
...
def queue_video(
    ...
    return api_json("/video/queue", "POST", payload, api_key, timeout=60)
...
def retrieve_video(
    ...
    try:
        ...
        # DNS failures, refused connections and read timeouts are not
        # HTTPError; they need wrapping too, as venice_common.make_request
        # does, or a mid-poll blip surfaces as a traceback.
...
def complete_video(api_key: str, model: str, queue_id: str) -> bool:
    ...
    data = api_json("/video/complete", "POST", payload, api_key, timeout=30)
...
def main() -> int:
    ...
    # Created lazily at download time, so a queue or poll failure leaves
